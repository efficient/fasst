echo "Removing throughput records from losses/"
rm -f losses/out-worker-*
echo "Removing reordering records from out/"
rm -f out/*
...
for i in `seq 1 26` `seq 28 70`; do
	...
	ssh -oStrictHostKeyChecking=no akalianode-$i\.RDMA.fawn.apt.emulab.net "cd /users/akalia/hots/app/swarm/; ./run-servers.sh $server_id" &
