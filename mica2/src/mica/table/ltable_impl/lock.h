#pragma once
#ifndef MICA_TABLE_LTABLE_IMPL_LOCK_H_
#define MICA_TABLE_LTABLE_IMPL_LOCK_H_

namespace mica {
namespace table {
template <class StaticConfig>
void LTable<StaticConfig>::lock_bucket(Bucket* bucket) {
...
template <class StaticConfig>
void LTable<StaticConfig>::unlock_bucket(Bucket* bucket) {
  if (StaticConfig::kConcurrent) {
...
template <class StaticConfig>
void LTable<StaticConfig>::lock_extra_bucket_free_list() {
  if (StaticConfig::kConcurrent) {
...
template <class StaticConfig>
void LTable<StaticConfig>::unlock_extra_bucket_free_list() {
  if (StaticConfig::kConcurrent) {
    if (concurrent_access_mode_ == 2) {
      ::mica::util::memory_barrier();
...
template <class StaticConfig>
uint32_t LTable<StaticConfig>::read_version_begin(const Bucket* bucket) const {
  if (StaticConfig::kConcurrent) {
    if (concurrent_access_mode_ != 0) {
      while (true) {
...
template <class StaticConfig>
uint32_t LTable<StaticConfig>::read_version_end(const Bucket* bucket) const {
  if (StaticConfig::kConcurrent) {
    if (concurrent_access_mode_ != 0) {
      ::mica::util::memory_barrier();
      uint32_t v = *(volatile uint32_t*)&bucket->version;
